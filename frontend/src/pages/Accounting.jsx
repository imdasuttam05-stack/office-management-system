...
export default function Accounting() {
  ...
  const [group, setGroup] = useState({ name: "", parent: "", nature: "Expense" });
  ...
  async function saveGroup(e){e.preventDefault();try{await post('/api/accounting/groups',group);setGroup({name:"", parent:"", nature:"Expense"});setMessage('Group saved');load()}catch(e){setError(e.message)}}
  ...
  return <div style={styles.page}>
    ...
    {tab==='groups' && <div style={styles.grid}><form style={styles.card} onSubmit={saveGroup}>
<h2>Create Group</h2>
<input required placeholder="Group name" value={group.name} onChange={e=>setGroup({...group,name:e.target.value})}/>
<select value={group.parent} onChange={e=>setGroup({...group,parent:e.target.value})}>
<option value="">Under: Primary</option>
{groups.map(g=><option key={g._id} value={g._id}>{`Under: ${g.parent?.name ? g.parent.name + " → " : ""}${g.name}`}</option>)}
</select>
<select value={group.nature} onChange={e=>setGroup({...group,nature:e.target.value})}>
{['Asset','Liability','Income','Expense','Capital'].map(x=><option key={x}>{x}</option>)}
</select>
<button>Save Group</button>
</form><div style={styles.card}><h2>Accounting Groups</h2><table><thead><tr><th>Group</th><th>Under</th><th>Nature</th></tr></thead><tbody>{groups.map(g=><tr key={g._id}><td>{g.name}</td><td>{g.parent?.name || "Primary"}</td><td>{g.nature}</td></tr>)}</tbody></table></div></div>}
