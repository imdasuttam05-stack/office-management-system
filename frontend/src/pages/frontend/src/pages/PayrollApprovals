...
export default function PayrollApprovals() {
  ...
  async function approveSelected() {
    ...
    if (!approveOT && !approveCutting) {
      setError(
        "Select Overtime or Cutting approval."
      );
      return;
    }
    ...
    try {
      ...
      const response =
        await hrApi.approvePayrollAdjustments({
          ids: selected,
          overtimeApproved: approveOT,
          cuttingApproved: approveCutting
        });
