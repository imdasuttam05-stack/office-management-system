...
function SelectField({ label, name, value, onChange, options, required = false }) {
  return <Field label={label} name={name} value={value} onChange={onChange} required={required}><select name={name} value={value} onChange={onChange} required={required} style={S.input}><option value="">Select {label}</option>{(Array.isArray(options) ? options : []).map(x => <option key={x} value={x}>{x}</option>)}</select></Field>;
}
...
export default function Employees() {
  ...
  async function load() {
    try { setError(""); const [e, o, sh] = await Promise.all([hrApi.employees(), hrApi.options(), hrApi.shifts()]); setRows(e.employees || []); setOptions({ companies: [], departmentOptions: [], designationOptions: [], employeeTypeOptions: [], states: [], settings: {}, ...(o || {}) }); setShifts(Array.isArray(sh?.shifts) ? sh.shifts : []); setRates(o.settings || rates); }
  ...
  return <main style={S.page}>
    ...
    <form onSubmit={save} style={S.card}>
      <Section title="Personal & Company Details">
        ...
        <Field label="Company Name" name="companyName" value={form.companyName} onChange={update} required><select name="companyName" value={form.companyName} onChange={update} required style={S.input}><option value="">Select Company</option>{(Array.isArray(options.companies) ? options.companies : []).map(x => <option key={x}>{x}</option>)}</select></Field>
