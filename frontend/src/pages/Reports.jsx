...
function SalaryReports() {
  ...
  return <section>
    ...
    {slip && <SalarySlipModal slip={slip} mailNotice={mailNotice} onClose={()=>{setSlip(null);setMailNotice(null)}} onPrint={printSlip} onExcel={()=>hrApi.exportSalarySlip(slip.salaryId, `salary-slip-${slip.employeeCode}-${month}-${year}.xlsx`).catch(e=>setError(e.message))} onEmail={async(to)=>{setMailNotice({type:"sending",message:`Sending salary slip to ${to}...`});try{const r=await hrApi.sendSalarySlipEmail(slip.salaryId,to);setMailNotice({type:"success",message:r.message || `Salary slip sent successfully to ${to}.`});}catch(e){setMailNotice({type:"error",message:e.message || "Salary slip email failed."});}}} />}
  </section>;
}

function SalarySlipModal({ slip, mailNotice, onClose, onPrint, onExcel, onEmail }) {
