...
function SalaryReports() {
  ...
  function printSlip(overrides = {}) {
    ...
    printHtml("Salary Slip", `<div class="slip"><div class="print-slip-head"><h1>${escapeHtml(printCompanyName)}</h1><p>${escapeHtml(printCompanyAddress)}</p><h2>Salary Slip - ${escapeHtml(slip.monthLabel)}</h2></div><table class="employee-info"><tbody><tr><td>Employee's Name</td><td>${escapeHtml(slip.employeeName)}</td><td>UAN No.</td><td>${escapeHtml(slip.uanNo)}</td></tr><tr><td>Designation</td><td>${escapeHtml(slip.designation)}</td><td>ESIC No.</td><td>${escapeHtml(slip.esiNo)}</td></tr><tr><td>DOB</td><td>${escapeHtml(slip.dob)}</td><td>Days</td><td>${slip.days}</td></tr><tr><td>DOJ</td><td>${escapeHtml(slip.doj)}</td><td>Paid Days</td><td>${slip.paidDays}</td></tr><tr><td>Bank Name</td><td>${escapeHtml(slip.bankName)}</td><td>LOP</td><td>${slip.lop}</td></tr><tr><td>IFSC Code</td><td>${escapeHtml(slip.ifscCode)}</td><td>Account No.</td><td>${escapeHtml(slip.accountNo)}</td></tr></tbody></table><table class="salary-breakdown"><thead><tr><th colspan="2">Actuals / Earnings</th><th colspan="2">Deductions</th></tr></thead><tbody><tr><td>Basic</td><td class="right">${money(e.basic)}</td><td>ESI Employee</td><td class="right">${money(d.esi)}</td></tr><tr><td>HRA</td><td class="right">${money(e.hra)}</td><td>PF Employee</td><td class="right">${money(d.pf)}</td></tr><tr><td>Conveyance</td><td class="right">${money(e.conveyance)}</td><td>Professional Tax</td><td class="right">${money(d.professionalTax)}</td></tr><tr><td>Others</td><td class="right">${money(e.others)}</td><td>Fine / Cutting</td><td class="right">${money(d.cutting)}</td></tr><tr><td>OT Approved</td><td class="right">${money(t.overtimeAmount)}</td><td>Other / LOP</td><td class="right">${money(d.other)}</td></tr><tr class="total-row"><th>Total Rs.</th><th class="right">${money(t.earnings)}</th><th>Rs.</th><th class="right">${money(t.deductions)}</th></tr></tbody></table><div class="net-card"><div><b>Total Net Payable</b><small>Net salary after approved deductions</small></div><b class="net-amount">${money(t.netPayable)}</b></div><div class="words-box"><b>In Words:</b> ${escapeHtml(slip.inWords)}</div><div class="salary-meta"><span>Actual OT: ${fmtHours(t.overtimeHours)}</span><span>Approved OT: ${fmtHours(t.overtimeApprovedHours)}</span><span>OT Amount: ${money(t.overtimeAmount)}</span><span>Actual Cutting: ${fmtMinutes(t.cuttingMinutes)}</span><span>Approved Fine: ${fmtMinutes(t.cuttingApprovedMinutes)}</span><span>Fine Amount: ${money(t.cuttingAmount)}</span></div></div>`, "@page{size:A4 portrait;margin:10mm}.slip{max-width:780px;margin:0 auto}.print-slip-head{text-align:center;border-bottom:2px solid #1d4f91;padding-bottom:12px;margin-bottom:14px}.print-slip-head h1{font-size:23px;margin:0;color:#172b4d}.print-slip-head p{margin:4px 0;color:#667085}.print-slip-head h2{margin:8px 0 0;font-size:17px}.employee-info,.salary-breakdown{width:100%;border-collapse:collapse}.employee-info{margin-top:8px}.employee-info td{border:1px solid #e3e8ef;padding:8px;font-size:11px}.employee-info td:nth-child(odd){background:#f7f9fc;font-weight:700;color:#344054}.salary-breakdown{margin-top:16px;border:1px solid #dfe6ef}.salary-breakdown th,.salary-breakdown td{border:1px solid #e3e8ef;padding:9px;font-size:11px}.salary-breakdown thead th{background:#eef4fb;color:#17365d;text-align:left;font-size:12px}.salary-breakdown .right{text-align:right}.salary-breakdown .total-row{background:#f4f7fb}.net-card{margin-top:15px;padding:13px 15px;background:#eef5ff;border:1px solid #cfe0fa;border-radius:8px;display:flex;justify-content:space-between;align-items:center}.net-card small{display:block;color:#667085;font-size:9px;margin-top:2px}.net-amount{font-size:18px;color:#17365d}.words-box{margin-top:11px;padding:10px 12px;background:#fafbfd;border:1px solid #e7ebf0;border-radius:7px;font-size:10px}.salary-meta{margin-top:10px;padding-top:8px;border-top:1px dashed #d6dde8;display:flex;flex-wrap:wrap;gap:5px 12px;color:#667085;font-size:9px}");
  }
